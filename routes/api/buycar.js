...
router.post("/",(req,res,next)=>{
    let {user_id,bianhao_id,num,newprice,pinpai,ico,kuanshi}=req.body
    console.log(user_id,bianhao_id,num)
    mgdb({
            ...
            collection.insertOne({
                ...
                if(!err){
                    if(result.result.ok>0){
                        res.send({err:0,mess:'加入成功'})
                    }else{
                        res.send({err:1,mess:'加入失败'})
                    }
                }else{
